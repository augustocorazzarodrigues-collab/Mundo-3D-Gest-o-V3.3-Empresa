...
export function computeMaterialCost(product, materials){
  const material = materials.find((m)=>m.id===product?.material_id || m.id===product?.materialId);
  if(!material) return 0;
  if(material.variable) return Number(product.manual_material_cost ?? product.manualMaterialCost ?? 0);
...
export function getProductCostTotal(db, productId, machineId){
  const product = byId(db.products, productId); if(!product) return 0;
...
export function getFinancialSummary(db){
  ...
  return { receitasPrevistas, receitasReais, despesas, saldoPrevisto: receitasPrevistas - despesas, saldoReal: receitasReais - despesas, entries };
}
