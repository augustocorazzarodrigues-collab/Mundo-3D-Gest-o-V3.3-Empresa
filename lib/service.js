...
async function ensureMutationAllowedForCurrentPath() {
  ...
  const baseRoutes = ROLE_ACCESS[role] || ROLE_ACCESS.viewer;
  const hasAll = baseRoutes.includes('*');
  const baseAllowed = hasAll || baseRoutes.some((route) => routeMatches(pathname, route));

  if (!baseAllowed) {
    throw new Error('Você não tem permissão para alterar nesta tela.');
  }

  const tabKey = resolveTabKeyFromPath(pathname);

  // Se a rota não estiver mapeada em uma aba específica, não bloqueia para não quebrar o app
  if (!tabKey) return;

  const permissions = await getCurrentTabPermissions();
  const level = permissions[tabKey] || 'view';

  if (level !== 'edit') {
    throw new Error(`Você tem permissão apenas de visualização na aba "${tabKey}".`);
  }
}
