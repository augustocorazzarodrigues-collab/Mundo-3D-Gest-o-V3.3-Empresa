...
export default function DashboardComercialPage() {
  ...
  useEffect(() => {
    loadAllDb()
      .then(setDb)
      .catch((e) => setError(e.message || 'Erro ao carregar dashboard comercial'));
  }, []);
  ...
  return (
    <AppShell>
      <Hero
        ...
        description="Visão de clientes, pedidos, pipeline e financeiro comercial usando dados do banco."
      />

      <section className="section cards-grid">
        <StatCard
          title="Receitas previstas"
          value={moneyBR(fin.receitasPrevistas)}
          description="Composição do comercial"
          icon="💸"
          toneClass="green"
        />
        <StatCard
          ...
          description="Pedidos vivos"
          icon="🧾"
          toneClass="blue"
        />
        <StatCard
          title="Leads abertos"
          value={(db.leads || []).filter((l) => !['Perdido', 'Fechado'].includes(l.stage)).length}
          description="Pipeline aberto"
          icon="🎯"
          toneClass="orange"
        />
        <StatCard
      ...
      <section className="section split-even">
        <BarPanel title="Status dos pedidos" subtitle="Leitura rápida da carteira" data={pedidos} />
        <BarPanel title="Pipeline" subtitle="Fases de prospecção" data={pipeline} />
      </section>
    </AppShell>
  );
}
